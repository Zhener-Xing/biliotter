...
- **浏览器扩展**：观看进度 / 字幕、专注中断检测、B 站登录态桥接（本机 `127.0.0.1:39261`）
- **笔记**：康奈尔笔记、一键整理、截图；本地 SQLite（按 uid 分库）
...
- **换号 / 退出**：尽量先 push，再卸挂载；**默认不删本地库**，下次同号可秒开，后台再增量 pull
- **云端**：JWT 按 uid 隔离；好友分享仅限好友关系

> 獭跟着账号走：人换了只卸挂载；人回来本地窝还在，再和云端对齐。
